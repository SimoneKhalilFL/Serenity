...
- **Houfy:** title field limit varies (60-char string fits comfortably in the observed field width; verify per-market before locking any longer variant). Houfy's audience most closely resembles VRBO's — high-consideration, family-and-group bookers who filter by guest count and city — so the same title works on both platforms (currently locked identical to the VRBO headline, character-for-character). Houfy is technically an OTA competitor positioning itself as "no-commission, owner-direct," so cross-OTA references by name in the title or Overview body read as either promotion of a competitor (`Airbnb`, `VRBO`, `Booking.com`) or protest-too-much cross-shopping defense (`Superhost`, `Premier Host`). Never reference competing OTAs by name in Houfy copy — trust signals stand on specifics (tenure, reply speed, owner-hosted). Never include `book direct` / `save fees` / `no service fees` in the Overview body — Houfy's own platform UI already surfaces the fee comparison. **URL slug trade-off:** Houfy may or may not allow slug edits on live listings; confirm in the dashboard. Same posture as Booking.com if slugs are locked — rebrand every user-visible copy field, accept the legacy slug.
