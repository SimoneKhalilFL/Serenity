...
- **Booking.com:** `Property Name` field is the title equivalent; practical budget ~65 chars for a comfortable read on the search-result card, ~70 chars typical extranet input maximum (verify per-market before locking). Booking.com surfaces the city on the search-result card via breadcrumbs but less prominently than Airbnb's tile subtitle — include the city in the property name. Include the bath count (`3BR/3BA` format) — Booking.com is hotel-style; bath count is a stronger filter signal here than on Airbnb / VRBO. Never reference competing OTAs by name in the title (`Airbnb`, `VRBO`, `Vrbo`, `Airbnb Superhost`, `Premier Host`, `Airbnb Plus`) — Booking.com's content-moderation flags cross-OTA references as potential channel steering. **URL slug trade-off:** Booking.com does not allow slug edits on live listings; accept the legacy slug on any rebrand-in-place pass and rebrand every other user-visible field. **Character-normalization constraint (portfolio-level rule, verified 2026-07-08 on TW2111 property-name paste):** Booking.com normalizes special characters in property names on publish — middots (`·` U+00B7) become spaces, hyphens in compound words (`Gulf-Front` → `Gulf Front`) become spaces, and forward slashes (`3BR/3BA` → `3BR 3BA`) become spaces. Airbnb, VRBO, and Houfy all preserve middots + hyphens + slashes verbatim; Booking is the only OTA in the portfolio that normalizes. Word content and brand-prefix identity are preserved (same words, same order); only visual rhythm changes. Submit the middot version anyway on the extranet paste — the field may still store what was submitted even when the display normalizes, and if Booking ever loosens the normalization policy, the ship string is already in the "correct" form. This normalization does NOT apply to the description body — the description-review workflow (Track B) accepts standard prose including punctuation. **Description-editing constraint (portfolio-level rule, verified 2026-07-08):** Booking.com is the only OTA in the portfolio where description prose is host-submitted-not-host-published — the "About this property" description body is owned and edited by Booking.com's content team, not the host. Hosts can directly edit the property name, amenity checkboxes, photos + captions, house rules, beds & rooms, location fields, and host profile; the description body requires a submission via the extranet's `Request a change to the description` / `Suggest a change` workflow (or via extranet Inbox message to Booking support) with ~5-10 business day turnaround, and Booking's content team edits ~30-40% of submitted phrasing to fit their house style. Treat Booking operationally different from Airbnb/VRBO/Houfy where paste-and-save is the standard workflow: on any Booking.com listing pass, split the ship into TRACK A (direct-paste fields, ~20 min) + TRACK B (description submit + wait). **Highest-ROI description lever** is not the review-workflow submission but structured-data completeness — Booking auto-generates description chunks from amenity checkboxes, bed configs, room types, property type, and location fields; the auto-description regenerates every 1-2 weeks and picks up structured changes automatically. Perfect all structured inputs first; file the review-workflow request only if the resulting auto-description still leaves a strategic-fact gap. (Verified 2026-07-06 via Booking.com extranet policy documentation; description-editing constraint discovered 2026-07-08 during TW2111 rebrand-in-place execution.)
- **Houfy:** description-title field renders 60–64 chars comfortably in the "About this place" H3 (verify per-market before locking any longer variant). Houfy's audience most closely resembles VRBO's — high-consideration, family-and-group bookers who filter by guest count and city. Houfy is technically an OTA competitor positioning itself as "no-commission, owner-direct," so cross-OTA references by name in the title or Overview body read as either promotion of a competitor (`Airbnb`, `VRBO`, `Booking.com`) or protest-too-much cross-shopping defense (`Superhost`, `Premier Host`). Never reference competing OTAs by name in Houfy copy — trust signals stand on specifics (tenure, reply speed, owner-hosted). Never include `book direct` / `save fees` / `no service fees` in the Overview body — Houfy's own platform UI already surfaces the fee comparison. **URL slug trade-off:** Houfy DOES allow slug edits on live listings (verified 2026-07-07 on TW2111 — legacy `/lodging/fun-in-the-sun/` successfully retargeted to `/h/twentyfirst`) — retarget legacy P1-violating slugs to brand-aligned slugs on any Houfy rebrand pass. **External URLs get silently stripped from the description body** (verified 2026-07-07 — a paste of `https://tidewaterhoa.com` was replaced with `[Removed]` on publish). Never paste external URLs into a Houfy Overview; either omit them or move them to the arrival-instructions message the host sends before check-in. **Two title-like fields:** Houfy renders both a page H1 (short listing name — separate editor field) AND a description H3 (from the "About this place" title). Verify BOTH carry the `Gulf-Front 3BR` differentiator, not just the description H3 — the H1 is what Google indexes and Houfy search tiles surface. **Reviews are imported from Airbnb + VRBO via platform-level feature** (confirmed 2026-07-07): the `Guest Reviews` section on any Houfy listing page displays reviews aggregated from that listing's Airbnb + VRBO archives, NOT reviews written natively on Houfy. Portfolio implications: (a) any Success Metric that counts Houfy reviews for a rebrand or trust-signal initiative must scope to Houfy-**native** reviews only — imported OTA reviews mirror trust from other platforms but don't reflect Houfy-native booking volume, and confusing the two produces a false-positive success signal; (b) any Houfy variant of the post-checkout review-solicitation email (Phase 3 initiative #44) must ask for a **Houfy-native** review explicitly since guests who already reviewed on Airbnb or VRBO may reasonably assume their prior review already counts; (c) source-of-truth for the actual review content stays in the Airbnb + VRBO archives per listing — do not create duplicate Houfy review archives, they will drift.
...
- **Primary ask is the review.** The canonical Send #1 SMS asks for a review AND includes the direct-book pitch inline (owner-baseline pattern preserved 2026-07-07 evening — reaches every guest, not just positive-reviewers, and stays out of any platform channel-steering issue since SMS is off-platform). The review ask lands before the direct-book pitch in the message body so the primary ask reads first. Alternative pattern: strip the direct-book pitch from Send #1 and fire it as a separate `Repeat-direct SMS` (HOSPITALITY.md §9) only after a positive review lands. Use the alternative only for specific high-value cases; default is inline.
- **One send per stay by default.** Send #1 (day +2) is the standard cadence. Send #2 (day +7 follow-up) and Send #3 (repeat-direct) are optional levers documented in HOSPITALITY.md §8 + §9, not standard practice. If you do use Send #2, one time only — never a third. Owner discretion on when to escalate to Send #2.
