...
- `[ ]` **Critical.** For TW2111 only: price calculator shows exactly **four** rows in order — `Nightly Rate`, `Cleaning Fee` ($250), `Taxes`, `Estimated Total`. No `Resort Registration Fee` line. No additional fee disclosure note beneath the calculator. *(Registration fee removed from the calculator 2026-07-06, Pricing/Logistics cleanup — MASTER §21 Placement rule. A cleaning-fee reduction to $200 was staged and reverted same session — held at $250.)*
- `[ ]` **Critical.** For TW2111 only: price calculator panel heading reads **`Your Stay`** (not `Price Calculator`); line labels read `Nightly Rate`, `Cleaning Fee`, `Taxes`; total row reads `Estimated Total`; and the trust note `No OTA service fees when booking direct.` renders directly below the total in muted (not highlighted) text. Cleaning Fee = **$250**.
