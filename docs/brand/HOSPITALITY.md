...
| **Direct-site (StayAtFlorida.com)** | `Google` **(deferred — see tracker #52 + [`GBP.md`](GBP.md))** | Once the brand-level Google Business Profile is created + verified (initiative #52 — full paste-ready profile + setup walkthrough in [`GBP.md`](GBP.md)), the SMS gets a direct `g.page/r/[REVIEW_CODE]/review` short link — one tap for the guest. Until then, direct-site guests default to whatever OTA the property has strongest visibility on *(TW2111: Airbnb — highest velocity + Superhost badge)*, framed as "Twenty First is also listed on Airbnb, and a quick review there would help other families find the home." |
