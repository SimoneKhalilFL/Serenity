...
>
> **Purpose:** Platform-ready copy for the Westlight Airbnb listing. The owner copies each block into the matching Airbnb editor field. **Cursor never publishes to Airbnb.**
>
...
**`Westlight · Gulf-Front 2BR`** *(26 / 50 chars — every character fits inside the mobile-tile-visible window)*
...
- **Brand-first** — `Westlight` occupies the first 9 characters. On the mobile tile scan, guests read the property brand before they read anything else. Reinforces the boutique-hospitality positioning against generic beach-title competition.
- **Differentiator second** — `Gulf-Front 2BR` is the two features guests actually filter on: "beachfront" (Gulf-Front) and bedroom count (2BR). Both fit inside the tile-visible window.
- **Miramar Beach dropped from the title** — Airbnb already surfaces the city ("Miramar Beach, FL") as a subtitle on tiles, so putting `Miramar Beach` in the title itself would duplicate what the platform is already showing for free. Trading that space for the differentiator is a win.
- **No fluff, no filler tail hook** — a "curiosity hook" tail (per 2026 title-engineering conventions) was considered but deferred: the brand + differentiator is already doing the click work, and adding a hook would push chars past the ~35 mobile-tile-visible cutoff.
- **Portfolio consistency** — Twenty First uses the same formula: `Twenty First · Gulf-Front 3BR`. A guest cross-shopping StayAtFlorida properties on Airbnb reads the same brand + Gulf-Front + bedroom-count pattern twice. That's the point.

**Owner sign-off:** 2026-07-09 evening.

**Never use:** floor number (8th, 8th-floor, floor 8), unit number (unit 811, MS811, condo 811), retired brand names (`Majestic Sun 811` as guest-facing brand, `Fun in the Sun`, `Serenity`, `Paradise`), all-caps, exclamation marks, `Book Now`, `Best deal`, or the resort name as a lead (`Majestic Sun · Westlight …`). The resort name appears only in the listing body's operational-context sentences, not the title.
...
|---|---|---|
| `Westlight · Beachfront Luxury on the Gulf` | 41 | Pre-launch draft. `Luxury` is soft language per the boutique-hospitality guardrails in [`../../brand/BRAND_GUIDELINES.md`](../../brand/BRAND_GUIDELINES.md). Reads slower on mobile scan than the differentiator formula. |
| `Westlight · 2BR Gulf-Front Family Retreat` | 41 | `Family Retreat` over-narrows the audience beyond what MASTER §4 supports (couples + small families + friend groups). |
| `Westlight · Miramar Beach Gulf-Front 2BR` | 40 | Duplicates the tile subtitle Airbnb already surfaces. Wastes prime real estate. |
| `Westlight — Gulf-Front 2BR` (em-dash variant) | 26 | Equivalent character count, em-dash instead of middot. Middot chosen for consistency with the VRBO / Booking / Houfy title separator pattern (single glyph across all four platforms). |
| `Westlight · Gulf-Front 2BR · Sleeps 6` | 37 | Considered — but the `Sleeps 6` tail crosses the ~35-char mobile-tile-visible window on older Airbnb tile UIs. Reserved for VRBO / Booking / Houfy where the title-field budget is more generous. |
| `Every Evening Ends in Gold · Westlight 2BR` | 42 | Tagline-lead variant. Rejected — the brand should lead the title, not the tagline (the tagline belongs in the summary and on-property page hero). |
...
- Primary bedroom with a king bed and Gulf-facing windows
- Guest bedroom with a queen bed, tucked away for privacy
...
The private Gulf-front balcony is the heart of Westlight. Morning coffee overlooking the Gulf. Afternoons to the sound of waves. Golden hour that gives the property its name. Direct beach access from the resort — no crossing a road. Complimentary beach chairs and umbrella available in the condo.
...
```
I'm the owner and I answer every message personally, typically within 2 hours. You'll get detailed arrival instructions about a week before your stay, and I'm a message away during your trip if anything comes up. I don't hover — but I'm here.
```
...
Airbnb forbids channel steering. **No `book direct` language anywhere in this listing.** The platform's native "Reserve" / "Request to Book" buttons are the CTA.

---

## Airbnb-specific rules

- Never mention `stayatflorida.com` in the listing body.
- Never mention `book direct`, `save fees`, or any OTA-comparison message.
- Never coach guests to mention our website URL in reviews — Airbnb flags this.
- Use Airbnb's own message templates for pre-arrival / check-out; the master wording lives in MASTER §20.
- **Legacy title `Majestic Sun 811` (in place prior to 2026-07-09) — the Airbnb dashboard change from that legacy title to `Westlight · Gulf-Front 2BR` is the property-level Signature elevation. Existing reviews stay attached to the same listing.**
