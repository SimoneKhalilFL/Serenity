...
| Beach access | Direct beach access via palm-lined boardwalk from the resort |
...
2. Direct beach access via palm-lined boardwalk — no crossing a busy road
...
- Direct beach access via palm-lined boardwalk — never crossing a busy road
...
| Beach access | `Direct beach access via palm-lined boardwalk` |
...
Same policy as TW2111 §23.

- **On-platform reviews (Airbnb, VRBO, Booking.com, Houfy):** display first name + last initial only, matching how the platform surfaces it. Example: `Andrew R.`
- **Direct-site (`stayatflorida.com` property page):** display the reviewer name as recorded in `config.js#REVIEWS[5]`. Legacy MS811 reviews (2020–2024) were captured before the SOP change and often show full last names. Do not backfill or edit these — they were consented at the time of collection. New reviews collected from 2026-07-06 onward follow the first-name + last-initial rule.
- **Never** display email addresses, phone numbers, city+state, or reservation IDs. Never publish a review the guest did not intend to be public (e.g. an inline text-message thank-you).
- **Property attribution** in review copy: legacy MS811 reviews may reference `Majestic Sun` inline (e.g. review #8: `Majestic Sun. 8th floor, great views`). Do not edit these — they are the guest's words. New review-response copy and any editorial framing (headers, section titles, "loved for" chips) must use `Westlight`.
