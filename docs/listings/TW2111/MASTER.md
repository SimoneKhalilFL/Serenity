...
- **Booking.com** *(≤~65 chars practical / 70 chars typical Booking.com property-name budget)*: **`Twenty First · Gulf-Front 3BR/3BA · Sleeps 8 · Panama City Beach`** *(64 chars)*. Canonical file: [`BOOKING.md`](BOOKING.md#property-name--locked-ship-string-phase-3-initiative-11-2026-07-06). Owner sign-off: 2026-07-06 evening. **Rebrand-in-place ship**: the previous live title `Oceanfront 3BR&3BA at Tidewater Resort - Pools - Sleeps 8` (58 chars) is retained in the BOOKING.md baseline table as the pre-rebrand audit trail. Note: the `/3BA` bath count is Booking.com-specific — Airbnb + VRBO do not include it. The first 29 chars still match the Airbnb + VRBO brand prefix character-for-character.
...
**Design principle across platforms:** the first 29 characters of every OTA title are identical (`Twenty First · Gulf-Front 3BR`). A guest cross-shopping the brand across Airbnb, VRBO, Booking.com (and, once shipped, Houfy) reads the same brand+differentiator every time, and recognizes the listing as one property with one owner. Platform-specific tails extend the brand prefix rather than replace it. Never introduce a platform-specific title that breaks this brand-prefix identity — the same brand must lead on every platform.
...
| 2026-07-06 | **Phase 3 initiative #5 + #6 tail-work — Airbnb + VRBO description body refresh (paste-ready)** | Follow-up to the #5 (Airbnb title) + #6 (VRBO headline) locks: description-body copy on both derivative files brought into alignment with the newly-locked title framing. **Change scope (deliberately tight — no rewrites of copy that's already tuned):** (1) **AIRBNB.md `Listing Summary`** — opener updated from `three-bedroom, three-bath beachfront retreat` → `three-bedroom, three-bath Gulf-front retreat` for title consistency; added `from the resort deck` to the beach-access phrase for specificity. New char count 447 / 500 (was 439). (2) **AIRBNB.md `Your Property (The Space)`** — opener `luxury beachfront retreat` → `Gulf-front retreat` (soft `luxury` adjective dropped per BRAND_GUIDELINES `OTA platform titles → Never in an OTA title` § which extends "if `Luxury` isn't earning its keep in the title, it shouldn't drift back in via the description body either"). (3) **VRBO.md `Short Description`** — `luxury 3-bedroom, 3-bath beachfront condo` → `3-bedroom, 3-bath Gulf-front condo`. New char count 384 (was 382). (4) **VRBO.md `Long Description → Welcome`** — same `luxury beachfront retreat` → `Gulf-front retreat` swap for consistency with #6. (5) **VRBO.md `About Your Host`** — added `I've been hosting Gulf-front stays on Florida's Emerald Coast for over six years` tenure line. Fills the gap left by VRBO's Premier Host badge (which doesn't surface tenure the way Airbnb's Superhost badge does — Airbnb's badge already reads *"Superhost, 6 years hosting"* so adding tenure to Airbnb's `Interaction with Guests` section would be redundant; skipping there deliberately). **Framing philosophy locked:** `Gulf-front` leads (once in the opener), then relaxes into `beachfront` in later paragraphs where readability variation helps — same pattern the newly-shipped BOOKING.md `Long Description` uses. Consistent brand voice across all four platforms (direct site uses `Gulf-front` in the property card + hero, OTAs now match). **No changes to:** AIRBNB `Guest Access`, `Interaction with Guests`, `Other Things to Note`, `Photo Captions`; VRBO `The Home`, `The View & Beach`, `Resort Amenities`, `The Neighborhood`, `About Your Stay`, `Amenities Section`, `Family Section`, `Location Section`, `House Rules Summary`, `Photo Captions`. Those sections were already well-tuned and this refresh deliberately does NOT re-open sections that don't need re-opening. **Owner action:** paste refreshed Airbnb `Listing Summary` + `Your Property (The Space)` into the Airbnb dashboard's matching fields; paste refreshed VRBO `Short Description` + `Long Description` (in particular the `Welcome` opener + `About Your Host` closer) into the VRBO dashboard's matching fields. Expected 10–15 minutes total across both platforms. No tracker status change — this is fold-in tail work on #5 + #6 (already flipped to 🟡 In Progress *(shipped, measuring)* earlier this session). **No `config.js`, `app.js`, `styles.css` changes.** | Content Sync Agent + Phase 3 #5/#6 tail-work (description body refresh) |
| 2026-07-06 | **Phase 3 initiative #11 — Booking.com rebrand-in-place TW2111 paste-ready** | Owner-approved via AskQuestion 2026-07-06 evening. Locked TW2111 Booking.com Property Name: **`Twenty First · Gulf-Front 3BR/3BA · Sleeps 8 · Panama City Beach`** (64 chars, under Booking.com's ~65-char practical property-name budget). First 29 characters match Airbnb #5 + VRBO #6 character-for-character (brand-prefix identity per BRAND_GUIDELINES `OTA platform titles → Brand prefix identity` rule). Owner picked the `/3BA` variant over the identical-to-VRBO alternate specifically to include the bath count for Booking.com's hotel-oriented search filters. **Baseline captured (audit trail)**: previous live title `Oceanfront 3BR&3BA at Tidewater Resort - Pools - Sleeps 8` overemphasized Tidewater, used awkward ampersand, no `Twenty First` brand differentiator. URL slug `fun-in-the-sun-panama-city-beach1` stuck — Booking.com does not allow slug edits on live listings; accepted trade-off in exchange for retaining the 3 verified reviews + 9.7/10 Exceptional aggregate. **Booking.com content gap flagged:** 3 reviews vs. 19 VRBO + 11 Airbnb — biggest platform-level content gap in the portfolio. Recovery lever is Phase 3 initiative #44 (post-checkout review-solicitation email); Success Metric on #11 is 3→6+ reviews within 90 days of #44 shipping. **MS811 Booking.com finding same session:** MS811 listing exists at `/hotel/us/deluxe-2-bedroom-beach-front-at-majestic-sun-resort.html` with **5.5/10 aggregate (Rated fair) + only 2 reviews** — a serious content-quality signal that title rewrites alone cannot fix. Per owner directive 2026-07-06 evening ("let's not worry about majestic sun right now. It will go thru it's own phase of rebranding once we are done with tidewater"), MS811 Booking.com rebrand deferred to initiative #1 (MS811 full rebrand pass). MS811 baseline captured in [`../MS811/BOOKING.md`](../MS811/BOOKING.md) with the `Deluxe`/`Majestic Sun Resort`/`Beach Front`/`8th floor` issues flagged for the future rebrand pass. **Bundled with the same commit — tracker housekeeping on #5 + #6:** owner confirmed the Airbnb + VRBO title strings were pasted into the respective host dashboards; #5 (Airbnb) + #6 (VRBO) flipped 📋 Planned *(paste-ready)* → 🟡 In Progress *(shipped 2026-07-06, measuring — Success Metric read date 2026-09-04, 60 days out — accounts for Airbnb + VRBO analytics lag on new titles)*. **Docs updates:** [`BOOKING.md`](BOOKING.md) rewritten end-to-end (new header, `Current live state` audit-trail section, locked ship string + rationale + alternates table, Short/Long/Host Profile refreshed with Gulf-Front framing + `6+ years hosting` tenure line, Booking.com-specific rules extended with brand-prefix identity + no-cross-OTA-references + URL-slug-trade-off + review-count-gap rules). [`../MS811/BOOKING.md`](../MS811/BOOKING.md) gains the captured baseline + explicit deferral to #1. [`MASTER.md`](MASTER.md) §12 OTA platform titles subsection updated with the locked Booking.com string (now three platforms locked, one pending — Houfy #12). [`../../brand/BRAND_GUIDELINES.md`](../../brand/BRAND_GUIDELINES.md) `OTA platform titles → Platform limits and prime real estate` bullet for Booking.com fleshed out (65-char budget, bath-count filter reasoning, no-cross-OTA-badge-references, URL-slug trade-off). Revenue-impact-tracker.md rows #5 + #6 + #11 updated with the new statuses + measurement dates; changelog entry appended. **No `config.js`, `app.js`, `styles.css` changes** — OTA copy work has no direct-site code surface. **Owner action for #11 ship:** open Booking.com extranet → Property tab → Property name → paste `Twenty First · Gulf-Front 3BR/3BA · Sleeps 8 · Panama City Beach` (verbatim, middot glyphs `·` U+00B7 included) → save. Then paste the refreshed Short Description + Long Description + Host Profile blocks from `docs/listings/TW2111/BOOKING.md` into the matching extranet fields. Expected 15–20 minutes total. Flip row #11 to 🟡 In Progress *(shipped, measuring)* with the ship date once the paste completes. | Content Sync Agent + Phase 3 initiative #11 (TW2111 rebrand-in-place paste-ready) + tracker housekeeping (#5 + #6 → 🟡 In Progress) |
