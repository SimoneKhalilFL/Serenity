...
| Cleaning fee | $250 | StayAtFlorida | Price calculator | Fixed per stay. *(A $250 → $200 reduction was staged on 2026-07-06 during the Pricing/Logistics cleanup pass, then reverted the same session before publishing per owner directive. Held at $250.)* |
...
**Status: HYBRID (2026-07-06).** Owner directive: use **real first names captured from the source OTA, no last initial**. Attribution string rendered on the Website is a unified `Verified guest` regardless of source platform. `config.js#REVIEWS[4]` carries the reviewer's real first name in the `author` field, the full OTA-captured name (first + last initial where applicable) in a non-rendered `sourceName` field for audit, and the source platform in a `platform` field for internal audit.
...
- Attribution string rendered by the site = **`Verified guest`** for all reviews, regardless of source platform. Do NOT branch the string on `platform`. The multi-platform trust signal is delivered via the review count and body detail, not via three distinct attribution labels. If this decision is ever reversed, branch in `app.js#renderReviewListItem` on `review.platform`.
...
- `platform` field on each entry drives no user-visible copy today. Preserved for internal analytics + JSON-LD `Review.publisher` if that markup is ever added.
...
**When reviewer name adjustments arrive.** If the owner reverses to `first + last initial`, update `author` values in `config.js#REVIEWS[4]` from the `sourceName` field, log the switch in the Changelog, and update this section. If the attribution style is reversed to per-platform (`Verified VRBO guest` / `Verified Airbnb guest` / `Verified Booking.com guest`), branch on `review.platform` in `app.js#renderReviewListItem` — the field is already stored.
...
- **Phase 3 OTA capture (2026-07-06) — CURRENT:** 30 real OTA reviews captured from VRBO + Airbnb + Booking.com public listings (archive at [`../reviews/`](../reviews/)). Enabled the hybrid policy above — real first names sourced from OTA, unified `Verified guest` attribution, no rating manipulation ever.
...
- **Reviewer name backfill: RESOLVED 2026-07-06.** Reviews now render with real first names sourced from the OTA public listings (VRBO 13 + Airbnb 10 + Booking.com 2 = 25 real max-rating reviews). See [§23 Review Author Naming Policy](#23-review-author-naming-policy) for the current hybrid policy. Follow-up items still open: (a) revisit whether to switch from unified `Verified guest` attribution to per-platform (`Verified VRBO guest` etc.), (b) revisit whether to re-add the aggregate rating chip on the property page (`hideReviewAggregate: true` today per owner directive), (c) decide whether to surface Simone's owner responses (2 on file — VRBO Shenna O. + Booking.com Stephanie).
