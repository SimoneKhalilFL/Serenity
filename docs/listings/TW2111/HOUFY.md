...
- **URL slug trade-off (pending confirmation).** Houfy may or may not allow slug edits on live listings; confirm in the dashboard. If editing is allowed, update `/lodging/fun-in-the-sun/` → `/lodging/twenty-first/` (matches the new title's brand). If not allowed, accept the legacy-slug trade-off (same posture as Booking.com) and rebrand every user-visible copy field only.
...
- **Review-volume gap.** Houfy TW2111 has 0 reviews on file. Recovery lever is Phase 3 initiative #44 (post-checkout review-solicitation email). Success Metric on #12 depends on #44 shipping — a rebrand-in-place pass alone will not move Houfy's search rank without content-quality signals to back it up.
